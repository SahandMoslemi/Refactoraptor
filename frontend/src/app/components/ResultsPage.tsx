...
const ResultsPage: React.FC<ResultsPageProps> = ({
    ...
    const handleEditorDidMount = (editor: any, monaco: any, ref: React.MutableRefObject<any>) => {
        ref.current = editor;
        ...
        if (ref === refactoredEditorRef && codeChanges) {
            setTimeout(() => {
                highlightLines(refactoredEditorRef, codeChanges.map(change => change.lineNumber));
            }, 500);
        }
    };
                    ...
                    <div className="w-full md:w-1/2 relative">
                        ...
                        <div style={{ height: "calc(100vh - 160px)" }}>
                            <Editor
                                ...
                                onMount={(editor, monaco) => handleEditorDidMount(editor, monaco, originalEditorRef)}
                            />
                        </div>
                    ...
                    <div className="w-full md:w-1/2 mt-4 md:mt-0 relative">
                        ...
                        <div style={{ height: "calc(100vh - 160px)" }}>
                            <Editor
                                ...
                                onMount={(editor, monaco) => handleEditorDidMount(editor, monaco, refactoredEditorRef)}
                            />

                            {/* Explanation overlay when showing */}
                            {showExplanation && (
                                <div className="absolute inset-0 bg-black/80 text-white p-6 overflow-auto">
                                    <h3 className="text-xl font-medium mb-4">Refactoring Explanation</h3>

                                    <div>
                                        <p className="mb-4">The following changes were made to improve the code:</p>
                                        <ul className="space-y-3">
                                            {codeChanges.map((change, index) => (
                                                <li key={index} className="p-2 rounded">
                                                    <span className="text-yellow-300">Line {change.lineNumber}:</span> {change.explanation}
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
